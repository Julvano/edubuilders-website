...
const Navbar: React.FC = () => {
  ...
  const aboutItems = [
    { name: content.about.visionMission.title, path: '/about/vision-mission' },
    { name: content.about.objectives.title, path: '/about/objectives' },
  ];

  const navItems = [
    { name: content.nav.home, path: '/' },
    { name: content.nav.actions, path: '/actions' },
    { name: content.nav.partners, path: '/partners' },
    { name: content.nav.publications, path: '/publications' },
  ];
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex justify-between h-20 items-center">
          ...
          <div className="hidden lg:flex items-center space-x-8">
            ...
            >
              <button className={`flex items-center gap-1.5 text-sm font-medium transition-colors hover:text-emerald-700 ${isDropdownOpen ? 'text-emerald-700' : 'text-gray-600'}`}>
                {content.nav.join}
            ...
            <button
              ...
              className="flex items-center space-x-2 px-3 py-1.5 rounded-full border border-gray-200 text-sm font-medium hover:bg-gray-50 transition-all ml-4"
            >
            ...
            >
              <button className="flex items-center gap-1.5 text-sm font-medium text-gray-600 transition-colors group-hover/about:text-emerald-700">
                {content.nav.about}
                <ChevronDown size={14} className="transition-transform duration-300 group-hover/about:rotate-180" />
              </button>
              <div className="absolute top-full right-0 mt-2 w-64 bg-white border border-gray-100 rounded-2xl shadow-xl p-2 opacity-0 invisible group-hover/about:opacity-100 group-hover/about:visible transition-all z-50">
                {aboutItems.map((item) => (
                  <NavLink
                    key={item.path}
                    to={langPath(item.path)}
